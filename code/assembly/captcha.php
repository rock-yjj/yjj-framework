...
class Captcha {
     ...
     * @return  bool
     */
    function _captcha($folder = '', $width = 145, $height = 20)
    {
        if (!empty($folder))
        {
            $this->folder = $folder;
    ...
    {
        $this->_captcha($folder, $width, $height);
    }
    ...
    {   
       // $_SESSION[$this->session_word] = base64_encode($this->encrypts_word($word));
	        //$_SESSION[$this->session_word] = md5($word);  //session 保存
	   
	   //setcookie($this->session_word,md5($word));
	   //验证码转为小写
	   $_SESSION[$this->session_word]=md5(strtolower($word));
    }
