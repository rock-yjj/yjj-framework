...
?>
      ...
      <span class="b_tt"><?php echo $current_nvg; ?></span>
				<span class="curbar"><i>您当前的位置：</i><a href="/">首页</a> > <a href="#">资质中心</a> > <i><?php echo empty($current_nvg) ? '' : '　>　<a href="#">' . $current_nvg . '</a>'; ?></i></span>
			</div>
...
foreach ($article_list as $k => $v) {
    ...
    $url = $this->_context->url('', array('controller' => 'honor', 'action' => 'article', 'sn' => $v['sn']));
    $img_url = $this->_context->imageIsExist(ROOT_DIR . '/public/upload/honor', $v['sn']);
    echo <<< EOF
				<li>
					<a href="{$url}" class="pic">
						<img src="/upload/honor/thumb_{$img_url}" alt="" class="vcenter" />
						<i></i>
					</a>
					<div class="text">
						<p>{$v['title']}</p>
...
<?php
if (!empty($pagination)) {
    $first_page = $this->_context->url('', array('controller' => 'honor', 'action' => 'list', 'page' => 1));
    echo '<a href="' . $first_page . '">首页</a>';
    for($i = $pagination['start']; $i <= $pagination['end']; $i++) {
        $url = $this->_context->url('', array('controller' => 'honor', 'action' => 'list', 'page' => $i));
        if ($i == $current_page) {
    ...
    $last_page = $this->_context->url('', array('controller' => 'honor', 'action' => 'list', 'page' => $page_count));
