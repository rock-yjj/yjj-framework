...
?>
      ...
      <span class="b_tt"><?php echo $current_nvg; ?></span>
				<span class="curbar"><i>您当前的位置：</i><a href="/">首页</a> > <a href="#">经典案例</a> > <i><?php echo empty($current_nvg) ? '' : '　>　<a href="#">' . $current_nvg . '</a>'; ?></i></span>
			</div>
...
foreach ($article_list as $k => $v) {
    ...
    $url = $this->_context->url('', array('controller' => 'special', 'action' => 'article', 'sn' => $v['sn']));
    $img_url = $this->_context->imageIsExist(ROOT_DIR . '/public/upload/special', $v['sn']);
    echo <<< EOF
				<li>
					<a href="{$url}" class="pic">
						<img src="/upload/special/thumb_{$img_url}" alt="" class="vcenter" />
						<i></i>
					</a>
					<div class="text">
						<p>{$v['title']}</p>
...
<?php
if (!empty($pagination)) {
    $first_page = $this->_context->url('', array('controller' => 'special', 'action' => 'type', 'page' => 1));
    echo '<a href="' . $first_page . '">首页</a>';
    for($i = $pagination['start']; $i <= $pagination['end']; $i++) {
        $url = $this->_context->url('', array('controller' => 'special', 'action' => 'type', 'page' => $i));
        if ($i == $current_page) {
    ...
    $last_page = $this->_context->url('', array('controller' => 'special', 'action' => 'type', 'page' => $page_count));
