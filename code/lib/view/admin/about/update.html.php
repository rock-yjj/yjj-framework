...
?>
            ...
            <p>
                ...
                <?php 
if ($img_url = $this->_context->imageIsExist(ROOT_DIR . '/public/upload/article', $article['sn'])) {
    echo '<img src="/upload/article/' . $img_url . '" width="118" height="84"/>';
}
                ?>
            </p>
            <p>
                <label>简介</label>
                <textarea name="article_info" cols=40 rows=5><?php echo empty($article['keywords']) ? '' : $article['keywords']; ?></textarea>
            </p>
            <p>
                <label>来源</label>
