...
<div class="container_12">
            <div class="grid_12">
                    ...
                    <a href="<?php echo $this->_context->url('', array('module' => 'admin', 'controller' => 'download', 'action' => 'add')); ?>" class="button">
                            <span>添加下载<img width="12" height="9" src="/admin/images/plus-small.gif" alt="New Product"></span>
                        </a>
                    </div>
                </div>
...
foreach ($download as $v) {
    ...
    $update   = $this->_context->url('', array('module' => 'admin', 'controller' => 'download', 'action' => 'update', 'sn' => $v['sn']));
    $delete   = $this->_context->url('', array('module' => 'admin', 'controller' => 'download', 'action' => 'del', 'sn' => $v['sn']));
    echo <<< EOF
        <tr class="{$line}">
            <td class="align-center">{$n}</td>
            <td><a href="{$v['download_url']}" target="_blank">{$v['title']}</a></td>
            <td>{$v['rank']}</td>
            <td>{$times}</td>
