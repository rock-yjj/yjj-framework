	...
	<div class="grid_12">
        ...
        <div class="module-body">
            ...
            <p>
                <label>用户名</label>
<?php 
    echo empty($passport['username']) ? '' : $passport['username'];
?>
            </p>
            <p>
                <label>密码</label>
...
<?php 
foreach ($role AS $k => $v) {
    if ($passport['role_sn'] == $v['sn']) {
        echo '<option value="' . $v['sn'] . '" selected="selected">' . $v['name'] . '</option>';continue;
    }
...
?>
            ...
            <p>
                <label>真是姓名</label>
                <input type="text" name="realname" id="realname" value="<?php echo empty($passport['realname']) ? '' : $passport['realname']; ?>" class="input-medium" style="width:200px" />
            </p>
            <p>
                <label>电话</label>
                <input type="text" name="tel" id="tel" value="<?php echo empty($passport['tel']) ? '' : $passport['tel']; ?>" class="input-medium" style="width:200px" />
            </p>
            <p>
                <label>邮箱</label>
                <input type="text" name="email" id="email" value="<?php echo empty($passport['email']) ? '' : $passport['email']; ?>" class="input-medium" style="width:200px" />
            </p>
 					<fieldset>
